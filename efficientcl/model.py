...
from efficientcl.common.masked_lm_utils import mask_tokens
from efficientcl.common.model_utils import all_gather_anchor_positive_pairs, unpack_batch
from efficientcl.losses import PyTorchMetricLearningLoss
from efficientcl.miners import PyTorchMetricLearningMiner
import logging
import torch.nn as nn
import random

@Model.register("efficientcl")
class EfficientCL(Model):
    """
    ...
    Registered as a `Model` with name "efficientcl".
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self._loss is None and not self._masked_language_modeling:
            raise ValueError(
                (
                    "No loss function provided. You must provide a contrastive loss (EfficientCL.loss)"
    ...
    def forward(  # type: ignore
        self, anchors: TextFieldTensors, positives: TextFieldTensors = None, difficulty: LabelField = None, num_lines: LabelField = None,
        ...
        if self.training:
            ...
            if self._loss is not None:
                if len(self.augment) == 0: 
                    ...
                    _, embedded_positives = self._forward_internal(positives, -1, self.curriculum, num_lines=num_lines, difficulty_step=difficulty_step)
                    ...
                    _, embedded_positives = self._forward_internal(anchors, augment, self.curriculum, num_lines=num_lines, difficulty_step=difficulty_step)
                    embedded_anchors, embedded_positives = all_gather_anchor_positive_pairs(
                        embedded_anchors, embedded_positives
                    )
    ...
    def _forward_internal(
        ...
        masked_lm_loss, embedded_text = self._text_field_embedder(tokens, augment, curriculum, num_lines, difficulty_step)
