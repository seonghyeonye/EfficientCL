...
class MLMTextFieldEmbedder(BasicTextFieldEmbedder):
    ...
    def forward(
        self, text_field_input: TextFieldTensors, augment: int, curriculum: str, num_lines: int, difficulty_step: int, num_wrapping_dims: int = 0, **kwargs
    ) -> torch.Tensor:
        ...
        for key in self._ordered_embedder_keys:
            ...
            if len(tensors) == 1 and len(missing_tensor_args) == 1:
                ...
                masked_lm_loss, token_vectors = embedder(
                    augment, curriculum, num_lines, difficulty_step, list(tensors.values())[0], **forward_params_values
                )
            else:
                ...
                masked_lm_loss, token_vectors = embedder(augment, curriculum, num_lines, difficulty_step, **tensors, **forward_params_values)
