...
class JiantRunner:
    ...
    def run_train(self, subset_ratio=None, index=None):
        for _ in self.run_train_context(subset_ratio, index):
            pass
    # def run_train(self):
    #     for _ in self.run_train_context():
    #         pass

    def run_train_context(self, subset_ratio=None, index=None, verbose=True):
        train_dataloader_dict = self.get_train_dataloader_dict(subset_ratio, index)
    ...
    # def resume_train_context(self, train_state, subset_ratio=None, verbose=True):
    #     train_dataloader_dict = self.get_train_dataloader_dict(subset_ratio)
    ...
    def get_train_dataloader_dict(self, subset_ratio=None, index=None):
    # def get_train_dataloader_dict(self):
        ...
        for task_name in self.jiant_task_container.task_run_config.train_task_list:
            ...
            train_dataloader_dict[task_name] = InfiniteYield(
                get_train_dataloader_from_cache(
                    train_cache=train_cache, task=task, train_batch_size=train_batch_size, subset_ratio=subset_ratio, index=index,
                )
                # get_train_dataloader_from_cache(
