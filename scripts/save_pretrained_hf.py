...
def main(archive_file: str, save_directory: str) -> None:
    ...
    EfficientCL model to a format that can be used with HuggingFace Transformers at `save_directory`."""
    save_directory = Path(save_directory)
    save_directory.parents[0].mkdir(parents=True, exist_ok=True)

    common_util.import_module_and_submodules("efficientcl")
    ...
    predictor = Predictor.from_archive(archive, predictor_name="efficientcl")
